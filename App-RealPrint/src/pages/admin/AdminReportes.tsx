...
interface PedidoPorServicio {
  servicio: string;
  cantidad: number;
  total: number;
}

interface DetallePorCaja {
  [key: string]: {
    pedidos: number;
    total: number;
  };
...
export default function AdminReportes() {
  ...
  const pedidosPorServicio: PedidoPorServicio[] = SERVICIOS.map((servicio) => ({
  ...
  const pedidosPorEstado: PedidoPorEstado[] = Object.entries(ESTADOS_PEDIDO).map(([key, { label }]) => ({
    estado: label,
    cantidad: pedidos.filter((p) => p.estado === key).length,
  }));

  // Top clientes
  const clientesAgrupados: Record<string, ClienteData> = pedidos.reduce((acc, p) => {
  ...
  const topClientes: ClienteData[] = Object.values(clientesAgrupados)
    .sort((a, b) => b.total - a.total)
    .slice(0, 5);

  // Inventario por categoría
  const inventarioPorCategoria: Record<string, InventarioCategoria> = inventario.reduce((acc, item) => {
    ...
    return acc;
  }, {} as Record<string, InventarioCategoria>);
                      ...
                      const productosServicio = productosFinales
                        ...
                        .map(pf => {
                          ...
                          return {
                            ...
                            detallePorCaja: stat?.detallePorCaja,
                          };
                        });
                      return (
                        <div key={servicio}>

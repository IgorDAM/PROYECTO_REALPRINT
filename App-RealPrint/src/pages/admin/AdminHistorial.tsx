...
export default function AdminHistorial() {
  ...
  const historialPedidos = (pedidos as PedidoItem[]).filter((p) =>
  ...
  const totalVentas = filteredPedidos.reduce((sum, p) =>
    p.estado !== "cancelado" ? sum + (typeof p.total === "number" ? p.total : 0) : sum, 0
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        <Input
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterFechaDesde(e.target.value)}
        />
        <Input
          type="date"
          value={filterFechaHasta}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterFechaHasta(e.target.value)}
        />
        <Button
          variant="secondary"
          onClick={() => {

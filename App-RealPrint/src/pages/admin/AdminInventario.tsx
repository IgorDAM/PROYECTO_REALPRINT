...
type InventarioItem = Record<string, any>;
...
interface TableColumn {
  key: string;
  label: string;
  render?: (value: any, row: InventarioItem) => JSX.Element | string;
}
...
export default function AdminInventario() {
  ...
  const [newItem, setNewItem] = useState<NewInventarioItem>({
    nombre: "",
    categoria: "",
    stock: 0,
    stockMinimo: 0,
  ...
  const filteredInventario = (inventario as InventarioItem[]).filter((item) => {
    const matchesSearch = String(item.nombre || "").toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stockBajo = (inventario as InventarioItem[]).filter((i) => i.stock <= i.stockMinimo).length;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <Select
          ...
          onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterCategoria(e.target.value)}
        ...
        {selectedItem && (
          <div className="space-y-4">
            <Input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedItem({ ...selectedItem, nombre: e.target.value })}
            />
            <Select
              id="edit-categoria"
              label="Categoría"
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setSelectedItem({ ...selectedItem, categoria: e.target.value })}
            />
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedItem({ ...selectedItem, stock: parseInt(e.target.value, 10) || 0 })}
              />
              <Input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedItem({ ...selectedItem, stockMinimo: parseInt(e.target.value, 10) || 0 })}
              />
              <Input
                id="edit-precio"
                label="Precio (€)"
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedItem({ ...selectedItem, precio: parseFloat(e.target.value) || 0 })}
              />
            </div>
            <label className="flex items-center gap-2 mt-2">
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setSelectedItem({ ...selectedItem, disponibleParaPedidos: e.target.checked })}
              />
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => setNewItem({ ...newItem, nombre: e.target.value })}
          />
          <Select
            id="add-categoria"
            label="Categoría"
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setNewItem({ ...newItem, categoria: e.target.value })}
          />
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Input
              id="add-stock"
              ...
              type="number"
              value={newItem.stock}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewItem({ ...newItem, stock: parseInt(e.target.value, 10) || 0 })}
            />
            <Input
              id="add-stock-minimo"
              label="Stock Mínimo"
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewItem({ ...newItem, stockMinimo: parseInt(e.target.value, 10) || 0 })}
            />
            <Input
              id="add-precio"
              label="Precio (€)"
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewItem({ ...newItem, precio: parseFloat(e.target.value) || 0 })}
            />
          </div>
          <label className="flex items-center gap-2 mt-2">
            <input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setNewItem({ ...newItem, disponibleParaPedidos: e.target.checked })}
            />
            <span>Disponible para pedidos</span>
          </label>
          <div className="block mb-1 mt-2">

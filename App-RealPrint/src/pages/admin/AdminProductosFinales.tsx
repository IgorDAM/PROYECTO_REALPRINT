...
interface NuevoProducto {
  ...
  talla?: string;
  materiales: Material[];
  clientesPermitidos: (string | number)[];
  precio: number;
  enCaja?: boolean;
  tamanoCaja?: number;
}
...
export default function AdminProductosFinales() {
  ...
  const columns: TableColumn[] = [
    ...
    { key: "prenda", label: "Prenda", render: (value: any, row: any) => row.prenda || "-" },
    { key: "modelo", label: "Modelo", render: (value: any, row: any) => row.modelo || "-" },
    { key: "talla", label: "Talla", render: (value: any, row: any) => row.talla || "-" },
    { key: "tamanoCaja", label: "Tamaño Caja", render: (value: any, row: any) => row.enCaja ? row.tamanoCaja : "-" },
  ...
  const handleAdd = () => {
    if (isEdit) {
      updateProductoFinal(editId as string | number, nuevoProducto);
    } else {
  ...
  productosFinales.forEach((pf: any) => {
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={isEdit ? "Editar Producto Final" : "Nuevo Producto Final"}
      >
        <div className="space-y-4">
          ...
          <Select
            label="Servicio"
            value={nuevoProducto.servicio}
            onChange={e => setNuevoProducto(prev => ({ ...prev, servicio: e.target.value }))}
            options={[
          ...
          {nuevoProducto.servicio === "serigrafia" && (
            <>
              <Select
                ...
                onChange={e => setNuevoProducto(prev => ({ ...prev, subservicio: e.target.value }))}
                options={[
              ...
              <Select
                ...
                options={[
                  { value: "1ª equipación", label: "1ª equipación" },
                  { value: "2ª equipación", label: "2ª equipación" },
                  { value: "3ª equipación", label: "3ª equipación" }
                ]}
                required
              />
              ...
              <Input
                ...
                type="number"
                min={1}
                value={nuevoProducto.tamanoCaja || 50}
                onChange={e => setNuevoProducto(prev => ({ ...prev, tamanoCaja: Number(e.target.value) }))}
              />
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {inventario.filter(prod => prod.serviciosDisponibles && prod.serviciosDisponibles.includes(nuevoProducto.servicio)).map((prod: any) => {
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setNuevoProducto(prev => {
                          ...
                          if (checked) {
                            if (!nuevos.find(m => m.id === prod.id)) {
                              nuevos.push({ id: prod.id, cantidad: 1 });
                            }
                          } else {
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {usuarios?.filter((u: any) => u.role === "cliente").map(cliente => (
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setNuevoProducto(prev => {
                        ...
                        if (checked) {
                          if (!actual.includes(String(cliente.id))) {
                            actual = [...actual, String(cliente.id)];
                          }
                        } else {
